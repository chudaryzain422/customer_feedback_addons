...
class SurveyController(Survey):
    ...
    #             if provider and provider.graph_api_authenticated:
    #                 template = request.env['wa.template'].sudo().search([('state', '=', 'added'),('provider_id', '=', provider.id),('name', '=', 'new')], limit=1)
    #                 if len(template.components_ids):
    #                     for line in template.components_ids:
    #                         if line.type == 'body':
    #                             line.text = f"Hello {partner.name}, thank you for your feedback! We have created a Discount coupon for you. Your coupon code is {coupon.code}."
    ...
    #                     'body': f"Hello {partner.name}, thank you for your feedback! We have created a Discount coupon for you. Your coupon code is {coupon.code}."
    #                 })
    #                 if compose:
    #                     compose.send_whatsapp_message()
    #                 # Example:
    #                 # whatsapp_api.send_message(partner.phone, message)
    #                 print(f"Sent WhatsApp message to {partner.mobile}")
    #         else:
    #             print(f"No phone number available for partner {partner.name}. WhatsApp message not sent.")
    #     except Exception as e:
